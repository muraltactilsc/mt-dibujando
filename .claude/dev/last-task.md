...
Smoke checks (via `dev-up.sh --web` + Playwright) confirmed: - fixture and non-existent emails both navigate to the same forgot-password confirmation; - the API log prints a real signed reset link; - `/reset-password?code=<valid>` renders the form, `/reset-password` alone shows the
invalid/expired screen; - mismatched passwords show the reset-specific message without a network call; - a valid reset navigates to confirmation, after which old password login fails and new
