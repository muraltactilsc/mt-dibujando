...
**Task 2 (auth reconstruction, no EntraID) is closed** (2026-07-11, PRs #8–#14): JWT bearer
login/refresh/logout/`/me` + role guard (PR #8) + login screen (PR #9), self-registration —
pre-registration eligibility quiz + account creation (PR #10 backend, PR #11 frontend, PR #12
real-content fix) — and forgot/reset-password (PR #13 backend, PR #14 frontend). See the
tracker's Task 2 section for the full decision record (JWT-over-cookies, the `/me`-endpoint
dual-state model, the `auth_sessions` table) and every deliberate fix shipped along the way: a
`turbo.json` pipeline bug, the guessable registration-quiz token, the unsigned password-reset
expiry, and a user-enumeration leak in `ForgotPassword` — each fixed and named, not silently
ported or dropped. Only the CRM-driven `OSCNotApproved → OSCApproved` login-promotion sub-piece
stays deferred (named blocker: CRM integration task not built yet). **Next**: the feature tracks
in the tracker's order — OSC registration wizard's non-CRM sections
(InstitutionalBase/LegalBase/Government/Finance) first, then Document/File management,
Announcement lifecycle, User admin, IFrame widgets, CRM integration, FunctionsDibujando's
background jobs.
